<template>
  <div class="content-upload-image-all">
    <div v-if="sImages" class="action-menu">
      <table-card-column-actions-menu
        v-if="bActiveModify"
        :aActionsTableCard="aActionsTableCard"
        :oItem="oItem"
        @setDetailItem="setOpenImgUpload"
      ...
      <div v-else class="content-empty-upload-new-image">
        <div v-if="bActiveModify" class="content-btn-empty-upload">
          <v-btn class="btn-primary-global" @click="setOpenImgUpload">
            <span class="mdi mdi-plus icon-plus-add"></span> Imagen
          </v-btn>
        </div>
        <core-img-empty />
      </div>
    ...
    <div v-if="false" class="content-btns-actions-upload-img">
      <div>
        <v-btn
          v-if="sImages"
...
export default {
  ...
  data: () => ({
    ...
    aActionsTableCard: [
      ...
      // },
      {
        sName: "reemplazar foto",
        sIcon: `<i class="mdi mdi-file-replace"></i>`,
        sAction: "replace",
        iAction: 1,
      },
    ],
