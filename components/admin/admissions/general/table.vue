<template>
  <div class="content-global-table">
    <p class="txt-title-table-global">
      <span class="font-weight-bold">Trámites</span> de Clientes
    </p>
      ...
      <template v-slot:item.sStatus="{ item }">
        <div class="d-flex justify-center">
          <div
            ...
            class="content-status-all font-weight-bold text-uppercase"
          >
            {{ item.sNameStatus }}
          </div>
        </div>
      </template>
      <template v-slot:item.sManager="{ item }">
        <p v-if="item.sManager">{{ item.sManager }}</p>
        <p v-else class="txt-empty-global">Sin información</p>
      </template>
      <template v-slot:item.sActions="{ item }">
        <v-btn
          @click="setDetailItem(item)"
          icon
          :class="item.oStyleAction.sClass"
        >
          <span :class="item.oStyleAction.sIcon"></span>
        </v-btn>
