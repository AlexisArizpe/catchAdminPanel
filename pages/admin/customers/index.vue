<template>
  <div class="content-page-all">
    <NuxtLayout name="panel">
      ...
      <headers-header sNameHeader="Consumidores" />
  ...
  methods: {
    async getItems() {
      try {
        ...
        const oResult = await this.$api.get(`customers`, payload);
        this.aItems = oResult.data.customers.map((e, i) => {
          return {
            idx: i,
            sId: e.sCustomerId,
            sLastActivity: "Falta de bk",
            sName: `${e.User.sName} ${e.User.sLastName}`,
            iActive: e.iActiveInteractions,
            iRedeemed: e.iRedeemedInteractions,
            iCompleted: e.iCompletedInteractions,
            iCanceled: e.iCanceledInteractions,
            bPlatformAccess: e.User.bPlatformAccess,
            sPlatformAccess: e.User.bPlatformAccess ? "Activo" : "Bloqueado",
          };
        });
        this.iNumPage =
